...
def test_length(input_value):
    ...
    # Get the word count and character count from the input_value
    word_count = spellcheck.word_count(input_value)
    char_count = spellcheck.char_count(input_value)


    # Check if word count is less than 10
    assert spellcheck.word_count(input_value) < 10

    # Check if character count is less than 50
...
def test_struc(input_value):
    ...
    """
    ### WRITE SOLUTION CODE HERE
    # Get the first and last characters from the input_value
    first_character = spellcheck.first_char(input_value)
    last_character = spellcheck.last_char(input_value)
